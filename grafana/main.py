...
def capture_1min():
	""" Executé pour capturer la temperature et CCS toutes les 1 min """
	global adc
	# temperature
	mv = (adc.read_u16() * 1.5 )/65.635
	# print( mv )
	mv -= 50 # There is an offset of 50 mv on the ADC
	# print( mv )
	tmp =  (mv-500)/10 
        tmp -= 4.42 # calibrating the sensor -> temp offset

	temp = "lmp_temp,loc=cabane temp={0:.2f}".format(tmp) # temp °C
	q.publish( "telegraf/Maison", temp )
